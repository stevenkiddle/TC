...
for (i1 in 1:5){
  ...
  sim <- simulateExp(tps = multiTS1@tps,num_time_series=421,shift_range=as.numeric(theta_ranges[i1,2:3]),exp_drop_par =  array(c(29,theta_ranges[i1,1]),dim=c(1,2)),num_clusters = 1)
...
file_name <- paste("paq_sim/sim",iter,".RData",sep="")
