...
sim_df <- data.frame(paq_aac=c(rep("PAQUID",50),rep("AAC",50)),label_swap=logical(100),theta1_1=NA,theta1_2=NA,theta2_1=NA,theta2_2=NA)
...
for (i in 1:length(files)){
  ...
  load(files[i])
    ...
    if (dist(rbind(thetas[theta_num,c(2,4)],simRes@params[c(2,4)])) < dist(rbind(thetas[theta_num,c(2,4)],simRes@params[c(4,2)]))) { 
      ...
      sim_df[k,3:6] <- simRes@params
      
    } else {
    
      sim_df[k,c(5,6,3,4)] <- simRes@params
