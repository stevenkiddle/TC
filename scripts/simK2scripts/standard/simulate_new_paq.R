...
sim <- simulateExp(tps = mmse@tps,num_time_series=421,exp_drop_par = t(array(thetas[sim_num,],dim=c(2,2))),shift_range=as.numeric(shift_ranges[sim_num,]),num_clusters=2)
...
range <- 3
  ...
  simRes <- new("TCsimRes", params = c(tmp[[2]][[1]]$par,tmp[[2]][[2]]$par), shifts = data.frame(real_shift=sim@shifts,est_shift=tmp[[3]]), clusters = data.frame(real_clusters=sim@clusters,est_clusters=tmp[[1]]),loss = discrimK2(data = sim@mat_data,tps = sim@mat_tps,result = tmp,traj_range=c(-ranges[range],ranges[range])))



if (!file.exists("paq_sim")){dir.create("paq_sim")}
